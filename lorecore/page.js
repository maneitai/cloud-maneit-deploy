...
const state = {
  ...
  selectedBookId: null, selectedSessionId: null, activeChapterId: null,
...
function renderBookList() {
  ...
  el.innerHTML = state.books.map(b => `
    <button class="book-card ${b.id === state.selectedBookId ? "book-card--active" : ""}"
...
function renderEntityList(tab) {
  ...
  if (!items.length) {
    el.innerHTML = `<div class="lib-placeholder muted">${state.selectedBookId ? `No ${tab} yet.` : "Select a book."}</div>`;
    return;
  }
...
const ENTITY_PANEL_FIELDS = {
  ...
  world: [
    ...
    { id:"ep_factions", label:"Factions",    type:"textarea" },
  ],
  scene: [
    { id:"ep_name",    label:"Title",       type:"input"    },
    { id:"ep_desc",    label:"Description", type:"textarea" },
    { id:"ep_pov",     label:"POV",         type:"input"    },
    { id:"ep_beats",   label:"Beats",       type:"textarea" },
...
function closeChapterEditor() {
  state.activeChapterId = null;
  if (qs("#chapterEditorEmpty")) qs("#chapterEditorEmpty").style.display = "block";
...
const QC_FIELDS = {
  characters: [
    { id:"qc_name", label:"Name",        type:"input",    p:"Character name" },
    { id:"qc_role", label:"Role",        type:"input",    p:"Protagonist / antagonist…" },
    { id:"qc_desc", label:"Description", type:"textarea", p:"Motivation, arc, personality" },
  ],
  worlds: [
    { id:"qc_name", label:"Name",        type:"input",    p:"World / setting name" },
    { id:"qc_desc", label:"Description", type:"textarea", p:"Geography, rules, culture" },
    { id:"qc_tone", label:"Tone",        type:"input",    p:"Dark fantasy, hard sci-fi…" },
  ],
  scenes: [
    { id:"qc_name", label:"Title",       type:"input",    p:"Scene title" },
    { id:"qc_desc", label:"Description", type:"textarea", p:"Purpose, conflict, beats" },
    { id:"qc_pov",  label:"POV",         type:"input",    p:"POV character" },
  ],
  chapters: [
    { id:"qc_name",  label:"Title",    type:"input",    p:"Chapter title" },
    { id:"qc_order", label:"Chapter #",type:"input",    p:"1" },
    { id:"qc_desc",  label:"Summary",  type:"textarea", p:"Optional opening / summary" },
  ],
  drafts: [
    { id:"qc_name", label:"Title",   type:"input",    p:"Draft title" },
    { id:"qc_desc", label:"Content", type:"textarea", p:"Draft content" },
  ],
  notes: [
    { id:"qc_name", label:"Title",   type:"input",    p:"Note title" },
    { id:"qc_desc", label:"Content", type:"textarea", p:"Note content" },
  ],
};
...
function updateQcLabel(tab) {
  const el = qs("#qcLabel");
  if (el) el.textContent = `New ${tabToSingular(tab)}`;
}
...
function openQuickCreate() {
  ...
  qs("#qcFields").innerHTML = defs.map(f => `
    <label class="inline-field" style="margin-bottom:6px;">
      <span class="soft">${escHtml(f.label)}</span>
      ${f.type === "textarea"
        ? `<textarea class="textarea" id="${f.id}" placeholder="${escHtml(f.p||"")}" rows="2"></textarea>`
        : `<input class="input" id="${f.id}" placeholder="${escHtml(f.p||"")}" />`}
    </label>
  `).join("");
...
function bindEvents() {
  ...
  qs("#entityList")?.addEventListener("click", e => {
    ...
    if (!btn) return;
    const tab = btn.dataset.entityTab;
    const id  = btn.dataset.entityId;
    const item = getList(tab).find(x => x.id === id);
